<script>
    ...
    let showPoseData = true;
  
    const FADE_DURATION = 10000; // 5 seconds
    const SHOW_DURATION = 10000; // 5 seconds
    ...
    function startAnimation() {
      ...
      const interval = setInterval(() => {
        drawFrame(frameCount % totalFrames);
        frameCount++;
  
        if (frameCount % (SHOW_DURATION / 1000 * poseData.fps) === 0) {
          showPoseData = !showPoseData;
          ctx.clearRect(0, 0, canvasElement.width, canvasElement.height);
        }
