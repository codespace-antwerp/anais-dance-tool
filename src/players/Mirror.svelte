<script>
    ...
    function drawLine(landmark1, landmark2) {
      ...
      // Mirror the line by connecting the mirrored points
      ctx.moveTo((1 - x1) * poseData.width, y1 * poseData.height);
      ctx.lineTo((1 - x2) * poseData.width, y2 * poseData.height);
    }
    ...
    function drawDot(landmark) {
      ...
      // Mirror the dot by drawing a mirrored dot
      ctx.moveTo((1 - x) * poseData.width, y * poseData.height);
      ctx.arc((1 - x) * poseData.width, y * poseData.height, 3, 0, 2 * Math.PI);
    }
