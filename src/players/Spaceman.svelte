<script>
    ...
    function drawFrame(frame) {
      ...
      // Set the background color based on the pose data value
      const poseDataValue = landmarks[LEFT_ELBOW_INDEX].z; // Change this to the appropriate pose data index
      const backgroundColor = poseDataValue > 10 ? "#FFFFFF" : "#D0F4DF";
      ...
      ctx.beginPath();
      drawDot(landmarks[LEFT_ELBOW_INDEX]);
      drawDot(landmarks[RIGHT_ELBOW_INDEX]);
    ...
    function drawLine(landmark1, landmark2) {
      let { x: x1, z: y1 } = landmark1;
      let { x: x2, z: y2 } = landmark2;
      ctx.moveTo(x1 * poseData.width, y1 * poseData.height + poseData.height / 2);
      ctx.lineTo(x2 * poseData.width, y2 * poseData.height + poseData.height / 2);
    }
  
    function drawDot(landmark) {
      let { x, z } = landmark;
      ctx.moveTo(x * poseData.width, z * poseData.height + poseData.height / 2);
      ctx.arc(x * poseData.width, z * poseData.height + poseData.height / 2, 3, 0, 2 * Math.PI);
    }
